...
float clamp(float f, float a, float b) { return max1(min1(f, b), a); }
...
vec3 add3(vec3 a, vec3 b) { vec3 r = { a.x + b.x, a.y + b.y, a.z + b.z }; return r; }//向量加
vec3 sub3(vec3 a, vec3 b) { vec3 r = { a.x - b.x, a.y - b.y, a.z - b.z }; return r; }//向量减
vec3 mul3(vec3 a, vec3 b) { vec3 r = { a.x * b.x, a.y * b.y, a.z * b.z }; return r; }//向量乘积
vec3 scale3(vec3 v, float s) { vec3 r = { v.x * s, v.y * s, v.z *s }; return r; }//数乘向量
float dot3(vec3 a, vec3 b) { return a.x * b.x + a.y * b.y + a.z * b.z; }//向量乘积值（用于求长）
float length3(vec3 v) { return sqrt(dot3(v, v)); }//求长
vec3 normalize3(vec3 v) { return scale3(v, 1.0f / length3(v)); }//单位化
...
float opI(float d1, float d2) { return max1(d1, d2); }
float opU(float d1, float d2) { return min1(d1, d2); }
float opS(float d1, float d2) { return max1(-d1, d2); }
...
void FullScreen() {
	...
	SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, x + 300, y + 300, NULL);
     MoveWindow(hwnd, -100, -400, x + 300, y + 300, 1);
}
...
float shading(vec3 v, vec3 n, vec3 eye) {
	...
	float rim = pow(1.0f - max1(-dot3(n, ev), 0.0f), 2.0f) * 0.15f;
...
float f(vec2 fragCoord,float x,float y) {
	vec3 dir = ray_dir(45.0f, fragCoord);
	vec3 eye = { 0.0f, 0.0f, 4.5f };
	mat3 rot = rotationXY(-x, y);
...
int main() {
	...
	while (1) {
		srand((int)time(0));
		float m = (float)(rand() % 20 / 10.0), n = (float)(rand() % 50 / 10.0);
		for (int y = 0; y < 80; y++) {
			for (int x = 0; x < 160; x++)
				putchar("  .,-:;+=zz*#@"[(int)(f(make2((x / 160.0f - 0.5f) * 2.0f, (y / 80.0f - 0.5f) * -2.0f),m,n) * 15.5f)]);
			putchar('\n');
		}
		Sleep(100);
		system("cls");
